...
class TrafficLight:
    def __init__(self, root):
        ...
        # Canvas
        self.canvas = tk.Canvas(root, width=200, height=350, bg='white', bd=2, relief=tk.GROOVE)
        self.canvas.grid(row=0, column=0, padx=20, pady=20, rowspan=3)

        # Traffic light frame
        self.canvas.create_rectangle(50, 40, 150, 290, fill='black', outline='gray')
        self.red_light = self.canvas.create_oval(75, 50, 125, 100, fill='gray')
        self.yellow_light = self.canvas.create_oval(75, 130, 125, 180, fill='gray')
        self.green_light = self.canvas.create_oval(75, 210, 125, 260, fill='gray')
        self.current_light = "red"
        self.update_light()

        # Enable touch interaction
        self.canvas.tag_bind(self.red_light, "<Button-1>", lambda event: self.turn_red())
        self.canvas.tag_bind(self.yellow_light, "<Button-1>", lambda event: self.turn_yellow())
        self.canvas.tag_bind(self.green_light, "<Button-1>", lambda event: self.turn_green())
        ...
        self.yellow_button = ttk.Button(control_frame, text="Yellow", command=self.turn_yellow, state=tk.DISABLED)
        self.yellow_button.grid(row=1, column=0, sticky="ew", pady=(0, 5))

        self.green_button = ttk.Button(control_frame, text="Green", command=self.turn_green, state=tk.DISABLED)
        self.green_button.grid(row=2, column=0, sticky="ew")
    ...
    def connect_serial(self):
        ...
        if selected_port:
            try:
                ...
                self.yellow_button.config(state=tk.NORMAL)
    ...
    def disconnect_serial(self):
        if self.arduino and self.arduino.is_open:
            ...
            self.yellow_button.config(state=tk.DISABLED)
    ...
    def update_light(self):
        ...
        self.canvas.itemconfig(self.yellow_light, fill=colors[self.current_light][1])
        self.canvas.itemconfig(self.green_light, fill=colors[self.current_light][2])
